...
neural_ode_layer = NeuralODE(dudt, (0.f0,1.f0), Tsit5(), solver_kwargs)  # DiffEqFlux.NeuralODE(dudt, (0.f0,1.f0), Tsit5(), solver_kwargs)
...
function accuracy(model,data; n_batches=100)
    ...
    for (x,y) in collect(data)[1:n_batches]
        target_class = classify(y)  # cpu(y))
        predicted_class = classify(model(x))  # cpu(model(x)))
...
dwn_loaded = model_cpu[1]  # gpu(model_cpu[1])
node_loaded= model_cpu[2]  # gpu(model_cpu[2])
...
slv = Flux.data(dense_done(dwn_loaded(x_train[1])))  # cpu(Flux.data(dense_node(dwn_loaded(x_train[1]))))
