module Utils 
using CSV, DataFrames, StatsBase, Flux, CuArrays, Plots
...
function get_curve(curves::GroupedDataFrame, idx::Union{Int, DataFrames.GroupKey}; start_frac=0.5, end_frac=1, norm::Bool=false)::Matrix
    # like pytorch dataset __getitem__
    curve = DataFrame(curves[idx])
    deletecols!(curve, :object_id)
    subset = copy(view_subset(curve; start_frac=start_frac, end_frac=end_frac))
